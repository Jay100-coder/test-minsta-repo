...
// if error message comes from first token fetch it will remove it from render
  ...
  if(firstTokenProps.addFilter === "By Owner"){
    const extension = process.env.NEXT_PUBLIC_NETWORK === "testnet" ? "testnet" : "near"

    const matchingOwner = firstTokenProps.newToken?.owner === `${firstTokenProps.ownerFilter}.${extension}`
    ...
    return !firstTokenProps?.newToken?.media || firstTokenProps?.isLoading ? (
      ...
      typeof firstTokenProps?.firstTokenisBlocked == "undefined") && matchingOwner === true ? (
      <MemoizedImageThumb
        key={firstTokenProps?.newToken?.media}
        token={firstTokenProps?.newToken}
        index={1}
